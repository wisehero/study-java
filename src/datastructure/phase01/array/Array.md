...
1. [배열의 본질](#1-배열의-본질)
2. [메모리 구조](#2-메모리-구조)
3. [O(1) 접근의 비밀](#3-o1-접근의-비밀)
4. [기본 타입 vs 참조 타입 배열](#4-기본-타입-vs-참조-타입-배열)
5. [배열 복사](#5-배열-복사)
6. [다차원 배열과 캐시 지역성](#6-다차원-배열과-캐시-지역성)
7. [총정리 요약](#-phase-1-1-배열array-총정리-요약)
...
# 📑 Phase 1-1: 배열(Array) 총정리 요약

기존의 비유를 덜어내고, 자바의 메모리 관리 원칙과 물리적 구조에 집중하여 전체 내용을 요약한다.

## 1. 배열의 탄생과 본질 (Phase 1-1-1)

**배열의 목적**: 흩어진 변수들을 하나의 연속된 메모리 공간으로 묶어 관리 효율과 반복 처리 능력을 극대화한다.

**핵심 특성**: 메모리 예약 방식의 연속성이 배열의 모든 장단점을 결정한다.

## 2. JVM 메모리 구조 (Phase 1-1-2)

**영역의 분리**: Stack 영역에는 Heap에 생성된 배열 객체의 **시작 주소(참조값)** 가 저장된다.

**물리적 연속성**: Heap 영역에서는 요청한 크기만큼의 메모리가 끊김 없이 이어져 할당된다.

## 3. $O(1)$ 접근의 수학적 비밀 (Phase 1-1-3)

**임의 접근(Random Access)**: `시작 주소 + (인덱스 × 요소 크기)` 공식을 통해 산술 연산만으로 목적지 주소를 즉시 계산한다.

**성능**: 배열의 크기와 무관하게 조회 속도가 일정하다($O(1)$). 이는 요소들을 하나씩 거쳐야 하는 연결 리스트($O(n)$)와 차별화되는 강점이다.

## 4. 데이터 타입에 따른 차이 (Phase 1-1-4)

**기본 타입(Primitive)**: 배열 공간에 실제 값이 직접 저장되어 메모리 접근이 단일 단계로 끝난다.

**참조 타입(Reference)**: 배열 공간에는 객체의 주소가 저장되며, 실제 객체는 Heap의 다른 위치에 존재한다. 데이터 접근 시 '주소 참조'라는 추가 단계가 발생한다.

## 5. 정적 크기의 한계와 복사 (Phase 1-1-5)

**배열 복사**: 크기 확장이 불가능하므로, 새로운 배열 생성 후 `System.arraycopy()`를 이용해 메모리 블록을 통째로 옮겨야 한다.

**얕은 복사**: 참조 타입 배열 복사 시 객체 자체가 아닌 '주소값'만 복사되어 두 배열이 동일 객체를 공유하게 됨을 주의해야 한다.

## 6. 하드웨어 최적화: 캐시 지역성 (Phase 1-1-6)

**공간 지역성**: CPU 캐시는 인접 데이터를 함께 로드하므로, 물리적으로 연속된 배열은 캐시 히트율이 압도적으로 높다.

**성능 차이**: 같은 $O(n^2)$ 연산이라도 메모리 배치 순서에 따라 행 우선 순회가 열 우선 순회보다 훨씬 빠를 수 있다.
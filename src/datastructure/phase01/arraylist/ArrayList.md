...
`ArrayList`의 모든 특징을 한눈에 볼 수 있도록 정리한다.

**인덱스 접근 ($O(1)$)**: 내부적으로 배열을 사용하므로 인덱스 기반 조회는 매우 빠르다.

**추가 연산 (Amortized $O(1)$)**: 용량이 부족하면 1.5배로 `grow()`한다. 이 덕분에 리스트 끝에 데이터를 추가하는 평균 비용은 상수 시간에 수렴한다.

**중간 삽입/삭제 ($O(n)$)**: 연속된 메모리를 유지하기 위해 요소들을 뒤로 밀거나 앞으로 당기는 Shifting 작업이 필수적이다.

**캐시 지역성**: 데이터가 물리적으로 인접해 있어 CPU 캐시 히트율이 높다. 이 덕분에 이론적인 시간 복잡도가 같더라도 실제 성능은 다른 자료구조보다 월등히 좋은 경우가 많다.